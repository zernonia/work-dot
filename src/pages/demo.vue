<template>
  <div class="w-auto mt-16 flex flex-col items-center">
    <div v-if="data.appStore" class="flex flex-col items-center">
      ...
      <div class="mt-24 flex items-center space-x-8">
        <img class="w-72 h-auto rounded-3xl" v-for="img in data.appStore.screenshots" :src="img" alt="" />
      </div>

      <div class="mt-24 max-w-screen-md">
        <p v-html="data.appStore.description" class="whitespace-pre-wrap text-xl text-gray-500 font-light"></p>
      </div>
    </div>
  </div>
</template>
